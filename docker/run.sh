...
if [ "${S3_ENABLED}" = "true" ]; then
    ...
    aws s3 cp --endpoint-url "${S3_ENDPOINT_URL}" \
        metadata.json "s3://${S3_BUCKET_NAME}/${RUN_DIR}/metadata.json"
    ...
    aws s3 sync --endpoint-url "${S3_ENDPOINT_URL}" \
        predictions/ "s3://${S3_BUCKET_NAME}/${RUN_DIR}/predictions/"
    
    # Upload output logs
    aws s3 cp --endpoint-url "${S3_ENDPOINT_URL}" \
        output-predict.log "s3://${S3_BUCKET_NAME}/${RUN_DIR}/output-predict.log"
    aws s3 cp --endpoint-url "${S3_ENDPOINT_URL}" \
