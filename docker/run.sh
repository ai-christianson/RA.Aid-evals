...
if [ "${ENABLE_EVAL}" = "true" ]; then
    ...
    # Upload evaluation files to S3 if enabled
    if [ "${S3_ENABLED}" = "true" ]; then
        ...
        # Sync predictions directory again
        aws s3 sync --endpoint-url "${S3_ENDPOINT_URL}" \
            predictions/ "s3://${S3_BUCKET_NAME}/${RUN_DIR}/predictions/" || echo "Warning: Failed to upload predictions"
        
        echo "Evaluation upload completed successfully"
    fi
else
    echo "Evaluation stage skipped (ENABLE_EVAL != true)"
fi
